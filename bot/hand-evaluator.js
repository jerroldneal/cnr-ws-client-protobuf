...
function preFlopStrength(holeCards) {
  ...
  let score;
  if (isPair) {
    // Pairs: 2x rank value, special cases for premiums
    score = hi * 2;
    ...
    score  = hi * 0.5;              // High-card value
    score += suited ? 2 : 0;        // Suited bonus
    score += gap <= 1 ? 1 : 0;      // Connector bonus
    score -= Math.max(0, gap - 3);  // Gap penalty
  }
 ...
 * Uses a combination of pre-flop strength and post-flop hand rank.
...
function winProbability({ holeCards = [], boardCards = [], numOpponents = 1 }) {
  ...
  if (boardCards.length === 0) {
    ...
    // Map hand rank 0–8 to rough heads-up equity
    const TABLE = [0.10, 0.38, 0.55, 0.68, 0.72, 0.77, 0.86, 0.93, 0.97];
    equity = result ? (TABLE[result.rank] ?? 0.5) : 0.5;
  }

  // Adjust for multiple opponents (each additional player ~reduces equity non-linearly)
